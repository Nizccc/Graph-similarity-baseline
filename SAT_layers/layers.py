...
from SAT_layers.gnn_layers import get_simple_gnn_layer, EDGE_GNN_TYPES
...
class AttentionModule(torch.nn.Module):
    ...
    def forward(self, x, batch, size=None):
        ...
        # print("transformed_global:", transformed_global)
        # print("transformed_global.shape:", transformed_global.shape)
        # print("transformed_global[batch]:", transformed_global[batch])
        # print("transformed_global[batch].shape:", transformed_global[batch].shape)


        coefs = torch.sigmoid(
            (x * transformed_global[batch]).sum(dim=1))  # transformed_global[batch]: 1128 * 16; coefs: 1128 * 0
...
class NTNNetwork(torch.nn.Module):

    def __init__(self, args, filters):

        super(NTNNetwork, self).__init__()
        self.args = args
    ...
    def setup_weights(self):
        ...
        self.weight_matrix = torch.nn.Parameter(
            torch.Tensor(
                self.filters, self.filters, self.args.tensor_neurons
            )
        )
        self.weight_matrix_block = torch.nn.Parameter(
            torch.Tensor(self.args.tensor_neurons, 2 * self.filters)
        )
