...
#--------------------eigenvalue---------
        path = osp.join(self.processed_dir, f'{self.name}_norm_eigenvalue.pt')
        if not os.path.exists(path):
            self.process()
        self.norm_eigenvalue = torch.load(path)

#--------------------degree---------
        path = osp.join(self.processed_dir, f'{self.name}_norm_degree.pt')
        if not os.path.exists(path):
            self.process()
        self.norm_degree = torch.load(path)
...
#--------------------------MCS ground-truth----------------------------------------
        ...
        with open(path, 'rb') as f:
            ...
            # mcss for (test_graph, test_graph) pairs are still float('inf'):
            x, y = torch.tensor(xs), torch.tensor(ys)
...
#--------------------------eigenvalue ground-truth----------------------------------------
        # Extracting ground-truth eigenvalues from the eigenvalue pickle file
        path = osp.join(self.raw_dir, self.name, 'eigenvalue.pickle')
        # Initialize eigenvalues as float('inf'):
        mat = torch.full((len(assoc), len(assoc)), float('inf'))
        with open(path, 'rb') as f:
            obj = pickle.load(f)
            xs, ys, gs = [], [], []
            for (x, y), g in obj.items():
                ...
                ys += [assoc[y]]
                gs += [g]
            # The pickle file does not contain eigenvalues for test graph pairs, i.e.
            # eigenvalues for (test_graph, test_graph) pairs are still float('inf'):
            x, y = torch.tensor(xs), torch.tensor(ys)
            eigenvalue = torch.tensor(gs, dtype=torch.float)
            mat[x, y], mat[y, x] = eigenvalue, eigenvalue
...
#--------------------------degree ground-truth----------------------------------------
        # Extracting ground-truth degrees from the degree pickle file
        path = osp.join(self.raw_dir, self.name, 'degree.pickle')
        # Initialize degrees as float('inf'):
        mat = torch.full((len(assoc), len(assoc)), float('inf'))
        with open(path, 'rb') as f:
            obj = pickle.load(f)
            xs, ys, gs = [], [], []
            for (x, y), g in obj.items():
                ...
                ys += [assoc[y]]
                gs += [g]
            # The pickle file does not contain degrees for test graph pairs, i.e.
            # degrees for (test_graph, test_graph) pairs are still float('inf'):
            x, y = torch.tensor(xs), torch.tensor(ys)
            degree = torch.tensor(gs, dtype=torch.float)
            mat[x, y], mat[y, x] = degree, degree
